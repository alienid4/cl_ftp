...
if (-not $DryRun) {
    ...
    # 授權 sftp_users 群組 (idempotent: 先查是否已加過)
    $existingAuth = Get-WebConfiguration -Filter "/system.ftpServer/security/authorization/add[@roles='sftp_users']" `
        -PSPath "IIS:\" -Location $siteName -ErrorAction SilentlyContinue
    if ($existingAuth) {
        Write-Host "[skip] 授權 sftp_users 群組已存在"
    } else {
        try {
            Add-WebConfiguration -Filter "/system.ftpServer/security/authorization" `
                ...
                -PSPath "IIS:\" -Location $siteName -ErrorAction Stop
            Write-Host "[ok  ] 授權 sftp_users 群組 Read+Write"
        } catch {
            Write-Host "[warn] 加入授權失敗 (可能已重複): $($_.Exception.Message.Split([char]10)[0])" -ForegroundColor Yellow
        }
    }
}
