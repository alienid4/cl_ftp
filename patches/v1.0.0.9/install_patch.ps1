<#
.SYNOPSIS
    [Patch v1.0.0.9] 通用 patch 安裝器 — 任意目錄都能跑。
.DESCRIPTION
...
.PARAMETER DryRun
    只列出將執行的動作, 不實際做事。
.EXAMPLE
    # 在 SF-PROJECT-ROOT/patches/v1.0.0.9/ 下跑 (自動偵測)
    .\install_patch.ps1
.EXAMPLE
    # 任意目錄下載解壓後跑, 拷到當前目錄
...
Write-Host "`n=== Install Patch v1.0.0.9 ===" -ForegroundColor Cyan
Write-Host "Round 4 修正: 路徑統一 (_portal vs DataExchange) + 7 個 deploy 腳本 bug" -ForegroundColor DarkCyan
...
switch ($PSCmdlet.ParameterSetName) {
    ...
    default {
        ...
        if (-not $ProjectRoot) {
            # 嘗試 1: 從腳本位置往上找 (預期結構: <root>/patches/v1.0.0.9/install_patch.ps1)
...
if (-not $DryRun) {
    Write-Host "✓ Patch v1.0.0.9 套用完成" -ForegroundColor Green
    Write-Host ""
