...
if (-not $SkipFeatures) {
    ...
    # OpenSSH (capability) — 加 try-catch 容錯
    Write-Host ""
    Write-Host "--- OpenSSH Server (FoD) ---"
    ...
    if ($sshCap -and $sshCap.State -eq 'Installed') {
        RecordStep 'OpenSSH.Server' 'skip' '已安裝'
    } elseif (-not $sshCap) {
        ...
        if ($DryRun) {
            ...
            try {
                ...
                Write-Host "    A. 公司 WSUS 包進 OpenSSH FoD package (找系管確認)"
                Write-Host "    B. 從 Windows installation media 帶 sxs source 過來:"
                Write-Host "       Add-WindowsCapability -Online -Name $($sshCap.Name) -Source <ISO-mount>\sources\sxs"
                Write-Host "    C. GUI 安裝 (短期可用):"
                Write-Host "       設定 → 應用程式 → 選用功能 → 新增功能 → OpenSSH Server"
                Write-Host "    D. 主機暫時開放 WU 連線, 裝完再關"
                Write-Host ""
