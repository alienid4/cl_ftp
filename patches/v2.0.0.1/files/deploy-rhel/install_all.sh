...
SF_SKIP_AD="${SF_SKIP_AD:-1}"
SF_SKIP_SAMBA="${SF_SKIP_SAMBA:-0}"
...
# v2.0.0.1: 業務代號帳號 (對齊主管圖 u0X 模型)
# 單一帳號 PoC: SF_ACCOUNTS="u01t"
# 多帳號:       SF_ACCOUNTS="u01 u02 u03"
SF_ACCOUNTS="${SF_ACCOUNTS:-u01t}"
SF_PASSWORD="${SF_PASSWORD:-}"   # 未指定不設密碼, 之後手動 passwd

export SF_DATA_ROOT SF_PORTAL_ROOT SF_AD_DOMAIN SF_AD_JOIN_USER SF_SKIP_AD SF_SKIP_SAMBA
export SF_PORTAL_PORT SF_DB_NAME SF_DB_USER SF_DB_PASS
export SF_ACCOUNTS SF_PASSWORD

step "SF File Exchange Server — RHEL 9 一鍵部署"
echo "DataRoot:   $SF_DATA_ROOT"
echo "PortalRoot: $SF_PORTAL_ROOT"
echo "Accounts:   $SF_ACCOUNTS $([ -n "$SF_PASSWORD" ] && echo '(密碼已設)' || echo '(未設密碼)')"
echo "AD Domain:  $SF_AD_DOMAIN (Skip=$SF_SKIP_AD)"
echo "Portal:     port $SF_PORTAL_PORT (HTTP only, 沒憑證)"
