...
if ($DryRun) {
    ...
    if (Test-Path $req) {
        # 找本地 wheels 目錄 (內網沒外網, 不能連 pypi.org)
        $wheelsDir = $null
        $searchPaths = @(
            (Join-Path $PSScriptRoot '..\python_wheels'),
            (Join-Path $PSScriptRoot '..\deploy\offline\python_wheels'),
            (Join-Path $PSScriptRoot '..\..\python_wheels'),
            (Join-Path $PSScriptRoot '..\..\deploy\offline\python_wheels'),
            'D:\install\python_wheels',
            'C:\ClaudeHome\python_wheels'
        )
        foreach ($p in $searchPaths) {
            if (Test-Path $p) {
                $found = Get-ChildItem $p -Filter '*.whl' -ErrorAction SilentlyContinue
                if ($found) { $wheelsDir = (Resolve-Path $p).Path; break }
            }
        }

        if ($wheelsDir) {
            Write-Host "[info] 使用本地 wheels: $wheelsDir"
            try {
                & $pip install --no-index --find-links $wheelsDir --upgrade pip 2>&1 | Out-Null
                & $pip install --no-index --find-links $wheelsDir -r $req
                Write-Host "[ok  ] 安裝 requirements.txt (offline 模式)"
            } catch {
                Write-Host "[warn] pip install 失敗: $($_.Exception.Message)" -ForegroundColor Yellow
            }
        } else {
            Write-Host "[warn] 未找到 python_wheels 目錄, 跳過 pip install" -ForegroundColor Yellow
            Write-Host "       (內網無外網, 不能連 pypi.org)" -ForegroundColor Yellow
            Write-Host ""
            Write-Host "之後手動取得 wheels:" -ForegroundColor Cyan
            Write-Host "  1. 外網 PC 跑:"
            Write-Host "     pip download -d wheels -r portal\requirements.txt"
            Write-Host "  2. USB 拷到 SF 主機 D:\install\python_wheels\"
            Write-Host "  3. 重跑 .\09_setup_portal.ps1"
        }
    }
}
