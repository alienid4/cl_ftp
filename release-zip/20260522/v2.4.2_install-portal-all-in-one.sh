#!/bin/bash
#
# v2.4.2_install-portal-all-in-one.sh — SF Portal 一次裝完
#
# v2.4.2 變更 (USER 跑 v2.4.1 起來但 /auth/login 顯 502):
#   - 補 10 個 Jinja2 templates → portal 不再 TemplateNotFound
#   - Step 5c 新增 templates 完整性檢驗 (10 個 .html 必須在, 否則 fail)
#   - Step 6b 強化: schema 套用後必須驗 6 個關鍵表都在, 不夠就 fail (不再 warn)
#   - Step 8 nginx default_server 衝突修正: 移掉 nginx.conf 內預設 server block
#
# 用法:
...
#   3. sf-portal-source-v2.4.2.tar.gz scp 到 /tmp/ftp-lab/

set -uo pipefail

VERSION="v2.4.2_install-portal-all-in-one (2026-05-22)"
...
if [[ ${#MISSING_TBL[@]} -gt 0 ]]; then
    echo ""
    echo "=== schema 套用最後輸出 ==="
    sudo -u postgres psql -d "$DB_NAME" -f "$SCHEMA_SQL" 2>&1 | tail -30
    fail "缺 ${#MISSING_TBL[@]} 個表 (${MISSING_TBL[*]}) — schema 套用失敗. 看上方錯訊"
fi
