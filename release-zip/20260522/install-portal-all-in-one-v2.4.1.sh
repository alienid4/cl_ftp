#!/bin/bash
#
# install_portal_all_in_one v2.4.1 — SF Portal 一次裝完
#
# v2.4.1 變更 (公司 GPT 接手 debug 找出新 root cause):
#   - Py 3.10 typing syntax `User | None` 在 RHEL 9 Py 3.9 不支援
#   - portal/app/auth.py 改用 `Optional[User]` (走新 tar.gz)
#   - Step 5 加 fallback Python patch — 若 USER 用舊 tar 也能自動修
#   - Step 8 nginx reload 失敗時 fallback 到 restart
#
# 用法:
...
#   3. sf-portal-source-v2.4.1.tar.gz scp 到 /tmp/ftp-lab/

set -uo pipefail

VERSION="install_portal_all_in_one v2.4.1 (2026-05-22)"
...
echo -e "${BOLD}${CYAN}║   修 Py3.9 typing + nginx reload fallback                    ║${NC}"
echo -e "${BOLD}${CYAN}╚══════════════════════════════════════════════════════════════╝${NC}"
...
# Portal source — 優先 v2.4.1, 再 v2.4.0, 再其他
PORTAL_SRC=""
for ver in v2.4.1 v2.4.0; do
    PORTAL_TAR=$(find /tmp /opt /root -maxdepth 5 -name "sf-portal-source-${ver}.tar.gz" -type f 2>/dev/null | head -1)
...
if [[ -z "$PORTAL_SRC" ]]; then
    MISSING_FILES+=("sf-portal-source-v2.4.1.tar.gz")
fi
...
if [[ -n "$SCHEMA_SQL" ]]; then
    ...
    if [[ "$TABLE_COUNT" -ge 6 ]]; then
        ok "Schema 套用成功 ($TABLE_COUNT/6 個關鍵表)"
    else
