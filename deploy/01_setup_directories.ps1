...
foreach ($d in $Departments) {
    ...
    foreach ($s in $subdirs) { $paths.Add(Join-Path (Join-Path $Root $d) $s) }
}
$paths.Add(Join-Path $Root '_portal')
foreach ($p in $portalDirs) { $paths.Add(Join-Path (Join-Path $Root '_portal') $p) }
